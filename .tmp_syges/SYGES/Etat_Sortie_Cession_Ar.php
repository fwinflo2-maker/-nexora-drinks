...
if (isset ($_SESSION['habilitation']) && ($_SESSION['habilitation']=="Administrateur" || $_SESSION['habilitation']=="Gerant" ))
{
 ...
 $pdf->AddPage();
 $pdf->SetTitle('ETAT DES SORTIES DE STOCK CESSION PAR AR');
 $pdf->SetAuthor('SYGES');
 $pdf->SetSubject('SORTIE STOCK');
 $pdf->Ln(5);
 $pdf->SetFont('arial','B',12);
 $pdf->Write(80,'                        ETAT DES SORTIES DE STOCK CESSION PAR ARTICLE');
 $pdf->Ln(3);
 $pdf->Write(80,'                        ____________________________________________________');
 $pdf->Ln(45);                                
 ...
 $pdf->Cell(30,7,'QTE SORTIE',1,1,'L',1);
 $pdf->SetFont('Arial','',8);
 $pdf->SetTextColor(0,0,0);
 $nbre=0;
 $TQTE=0;
...
$reponse2= $DataBase->query($sql2);
  ...
  $pdf->Cell(50,7,$marque.' '.$nbrebte,1,0,'L');
  $pdf->Cell(80,7,$libelle,1);
  $pdf->Cell(30,7,$qte,1,1,'C');
  $TQTE=$TQTE+$qte;
  $nbre++;
  }
  $pdf->SetFont('Arial','B',9);
  $pdf->Cell(70,7,'Nombre d\'article : '.$nbre,1,0,'C');
  $pdf->Cell(110,7,'Nombre de Colis : '.$TQTE,1,1,'C');
