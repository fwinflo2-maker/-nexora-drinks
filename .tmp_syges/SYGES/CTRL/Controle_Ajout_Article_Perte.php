		...
		// on verifie si cette vente n'est pas validée  
			...
			if ($rslt5["statut"]=='V')
			{
				?>
					...
					alert('Cette perte est déjà validée. ');
			...
			if($trve==true)
			{
				?>
					<script language="javascript" type="text/javascript">
					alert('Cet  article existe deja dans cette perte. ');
					history.back();
					</script>
				<?php
				exit();	
			...
			{
			//on recupere le prix de revient et d'achat de l'article
	...
	// insertion de l'article dans la bd
			...
			$sql="insert into articlevendu_frigo values (:codeart,:codevente,:qtevendu,:prixrevient,:prixvente,:observationvente)";
				...
				$insere2 = $req->execute(array(
											...
											'prixvente' =>$prixvente,
											'observationvente' =>$_POST['observationvente'],
											));
