<?php
	...
	if(isset($_POST['codeappro']))
	{
		//on se rassure de ce que la l'appro n'est pas validée
			...
			$sql5 = " select statut from approcession where id_appro='".$id."'";
			...
			if ($rslt5["statut"]=='V')
			{
				?>
					...
					alert('Cet Appro est déjà validée. ');
					window.location.replace("../index.php?formulaire=Choisir_ApproCession_Mod");
					</script>
				<?php
				exit();	
			}
		...
		$id=htmlentities(htmlspecialchars(strtolower($_POST["codeappro"])), ENT_QUOTES, 'UTF-8');
		$insere=0;
		$sql="update approcession set date_appro=:date_appro, observation=:observationappro where id_appro='".$id."'";
