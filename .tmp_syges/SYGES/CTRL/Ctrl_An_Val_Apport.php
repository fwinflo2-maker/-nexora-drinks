<?php
	...
	if(isset($_POST['Code']))
	{
		...
		if ($rslt5["statut"]=='N')
			{
				?>
					...
					alert('Cet apport est en instance. ');
					window.location.replace("../index.php?formulaire=Choisir_Apport_An_Val");
					</script>
				<?php
				exit();	
			}
		...
		$insere=0;
		$statut='N';
		$sql="update apport set statut=:statut where id_apport='".$code."'";
		...
		?>
				<script language="javascript" type="text/javascript">
				alert('Remise en instance de l\'apport effectuée.');
				window.location.replace("../index.php?formulaire=Choisir_Apport_An_Val");
				</script>
			<?php
			exit();
