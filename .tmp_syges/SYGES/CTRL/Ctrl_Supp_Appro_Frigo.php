<?php
	...
	if(isset($_POST['codeappro']))
	{
		...
		$supp=false;
		$sql4=" select * from approfrigo where id_appro='".$_POST['codeappro']."'";
		...
		if ($supp==true)
		{
			...
			$sql2='select id_article, id_appro from article_recu_frigo where id_appro="'.$id.'" ' ;
			$reponse= $DataBase->query($sql2);
			while($rslt= $reponse->fetch())
			{
				$sql3="delete from article_recu_frigo where id_appro='".$id."' and id_article='".$rslt['id_article']."'";
				$req2 = $DataBase->prepare($sql3);
				$insere2 = $req2->execute();
			}
			// suppression dans la bd
			$sql="delete from approfrigo where id_appro='".$id."'";
			$req = $DataBase->prepare($sql);
			$insere = $req->execute();	
?>
				<script language="javascript" type="text/javascript">
				alert('Suppression effectue');
				window.location.replace("../index.php?formulaire=Choisir_Appro_frigo_Supp");
				</script>
				<?php
				exit();
        }
