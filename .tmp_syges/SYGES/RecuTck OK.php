...
if (isset ($_SESSION['habilitation']))
{
 ...
 $pdf->Ln(10);
 $pdf->Write(25, utf8_decode('Facture : '));
  ...
  while($rslt2= $reponse2->fetch())
 {
  ...
  $pdf->Cell(45,20,number_format(number_format($tauxristournettc, 0, ',', ' ')*$rslt2['QTESORTIE'], 0, ',', ' '),1,1,'C');
  }
   		//ON CALCULE LES FRAIS D'ENLEVEMENT
  $ttfraisenlevement=0;
  $ttfraisenlevement=$fraisenlevement*$ttcolis;
  
  $pdf->Cell(225,20,'MT ARTICLES(1) : '.number_format($MT, 0, ',', ' '),1,1,'C');
 $pdf->Cell(105,20,'TOTAL COLIS : '.number_format($ttcolis, 0, ',', ' '),1,0,'C');
 $pdf->Cell(120,20,' TOTAL CASIERS  : '.number_format($nbrecasier, 0, ',', ' '),1,1,'C');
  $pdf->Cell(105,20,'TOTAL RIS. : '.number_format($MTR, 0, ',', ' '),1,0,'C');
  $pdf->Cell(120,20,'FRAIS ENLEV.(2) : '.number_format($ttfraisenlevement, 0, ',', ' '),1,1,'C');
 
 //affichage des consignes emballages
    $MTC=0;
	$ttcolisC=0;
	$pdf->Cell(225,20,'CONSIGNES',1,1,'C');
 ...
 $pdf->Cell(120,20,'MT CONSIG.(3) : '.number_format($MTC, 0, ',', ' '),1,1,'C');
  
    //affichage des deconsignations emballages
    $MTD=0;
	$ttcolisD=0;
	$pdf->Cell(225,20,'DECONSIGNATION',1,1,'C');
 ...
 $pdf->Cell(120,20,'MT DECON.(4) : '.number_format($MTD, 0, ',', ' '),1,1,'C');
  ...
  $pdf->Cell(225,20,'MONTANT ARTICLE HT : '.number_format($mtliquidenu, 0, ',', ' '),1,1,'C');
  $pdf->Cell(225,20,'TVA ('.$tva.'%) : '.number_format($mttva, 0, ',', ' '),1,1,'C');
  $pdf->Cell(225,20,'P.S.A ('.$tauxretfiscpro.'%) : '.number_format($mtretfiscpro, 0, ',', ' '),1,1,'C');
 $pdf->Cell(225,20,'SOUS-TOTAL (1)+(2)+(3)-(4) : '.number_format($MT+$MTC+$ttfraisenlevement-$MTD, 0, ',', ' '),1,1,'C');
 $pdf->Cell(225,20,'CREDIT RISTOURNE : '.number_format($creditristourne, 0, ',', ' '),1,1,'C'); 
  $pdf->SetFont('arial','B',32);
  $pdf->Cell(225,20,'NET A PAYER : '.number_format($MT+$MTC+$ttfraisenlevement-$MTD-$creditristourne, 0, ',', ' ').' FCFA',1,1,'C');
  $pdf->Ln(60);
