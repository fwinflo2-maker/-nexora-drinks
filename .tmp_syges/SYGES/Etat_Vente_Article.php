...
if (isset ($_SESSION['habilitation']) && ($_SESSION['habilitation']=="Administrateur" || $_SESSION['habilitation']=="Gerant" ))
{
 ...
 $pdf->Cell(25,7,'CONDITION.',1,0,'L',1);
 $pdf->Cell(45,7,' LIBELLE',1,0,'L',1);
 $pdf->Cell(25,7,'QTE',1,0,'C',1);
 $pdf->Cell(25,7,'PRIX REVIENT',1,0,'L',1);
 $pdf->Cell(25,7,'PRIX VENTE',1,0,'L',1);
 $pdf->Cell(35,7,'MARGE BRUTE',1,1,'C',1);
 $pdf->SetFont('Arial','',8);
 $pdf->SetTextColor(0,0,0);
...
$reponse2= $DataBase->query($sql2);
  ...
  $pdf->Cell(25,7,$marque.' '.$nbrebte,1,0,'L');
  $pdf->Cell(45,7,$libelle,1);
  $pdf->Cell(25,7,$qte,1,0,'C');
  $pdf->Cell(25,7,number_format($PRIXREVIENT, 0, ',', ' ').' F',1,0,'C');
  $pdf->Cell(25,7,number_format($PRIXVENTE, 0, ',', ' ').' F',1,0,'C');
  $pdf->Cell(35,7,number_format($BENEF, 0, ',', ' ').' F',1,1,'C');
...
while($rslt11= $reponse11->fetch())
  ...
  $pdf->Cell(25,7,' CASIER(S) : '.number_format($nbrecasier, 0, ',', ' '),1,0,'C');
  $pdf->Cell(45,7,'  NBRE ARTICLE(S) : '.number_format($nbre, 0, ',', ' '),1,0,'C');
  $pdf->Cell(25,7,' COLIS : '.$QTET,1,0,'C');
  $pdf->Cell(25,7,number_format($TTPR, 0, ',', ' ').' F',1,0,'C');
  $pdf->Cell(25,7,'(1)'.number_format($TTPV, 0, ',', ' ').' F',1,0,'C');
  $pdf->Cell(35,7,number_format($TTB, 0, ',', ' ').' F',1,1,'C');
  
  
  $pdf->Cell(60,7,' FRAIS ENLEVEMENT(2) : '.number_format($fraisenlevement, 0, ',', ' ').' F',1,0,'C');
  $pdf->Cell(65,7,' CONSIGNES CLIENTS(3) : '.number_format($mtcc, 0, ',', ' ').' F',1,0,'C');
  $pdf->Cell(75,7,' DECONSIGNATIONS CLIENTS(4) : '.number_format($mtrevte, 0, ',', ' ').' F',1,1,'C');
  
  $pdf->SetFont('arial','B',10);
  $pdf->Cell(200,7,' MONTANT TOTAL (1)+(2)+(3)-(4) : '.number_format($TTPV+$fraisenlevement+$mtcc-$mtrevte, 0, ',', ' ').' F',1,1,'C');
   //
  $pdf->SetFontSize(8);
  $pdf->Output(); 
}
