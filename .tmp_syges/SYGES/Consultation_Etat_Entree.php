...
<title>Consultation de l'etat des entrées en stocks</title>
</head>

<body>
<table id='liste' border="1" width="100%" align="center">
          <tr align="center" bgcolor="#CCCCCC">
          	<td colspan="4"><h3>Etat des entrées en stocks  </h3></td>
          </tr>
          <tr bgcolor="#CCCCCC">
                <td colspan="2" align="center" ><h5>Période </h5></td>
...
//Ici on recupre la liste sans doublons des articles livres dans la periode 
 $sql='SELECT DISTINCT  AR.ID_ARTICLE FROM ARTICLE_RECU AR, APPROVISIONNEMENT A  WHERE AR.ID_APPRO=A.ID_APPRO AND A.DATE_APPRO BETWEEN "'.$Debut.'" AND "'.$Fin.'" AND A.STATUT="V" ORDER BY AR.ID_ARTICLE  ';
$reponse= $DataBase->query($sql);
		while($rslt= $reponse->fetch())
		{
//Ici on recupere les quantites de la meme article puis on somme
$qterecu=0;
$sql1 = 'SELECT  AR.ID_ARTICLE, AR.QTERECU, AR.ID_APPRO, A.ID_APPRO, A.DATE_APPRO FROM ARTICLE_RECU AR, APPROVISIONNEMENT A WHERE AR.ID_APPRO=A.ID_APPRO AND A.DATE_APPRO BETWEEN "'.$Debut.'" AND "'.$Fin.'" AND AR.ID_ARTICLE="'.$rslt['ID_ARTICLE'].'" AND A.STATUT="V"';
...
?>
<tr>
	<td><a href="Etat_Entree.php?debut=<?php echo $_GET["DateD"];?>&fin=<?php echo $_GET["DateF"];?>"/><input type="button" value="Imprimer" /></td></a>
