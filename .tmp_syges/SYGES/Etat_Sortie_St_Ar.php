...
if (isset ($_SESSION['habilitation']) && ($_SESSION['habilitation']=="Administrateur" || $_SESSION['habilitation']=="Gerant"  || $_SESSION['habilitation']=="Comptable"))
{
 ...
 $pdf->Write(60,'                                     ETAT DES SORTIES DE STOCK');
 $pdf->Ln(1);
 $pdf->Write(60,'                                     ___________________________');
 $pdf->Ln(35);                                
 $pdf->SetFont('arial','B',10);
 $pdf->Write(2,'                     Article :  '.$libelle);
 $pdf->Ln(5);
 $pdf->SetFont('arial','B',10);
 ...
 $pdf->Cell(38,7,'OPERATION',1,0,'C',1);
...
$reponse1= $DataBase->query($sql1);
		...
		{
		  	$pdf->Cell(20,7,dateFormatFrancais($rslt1['DATE']),1);
		  	$pdf->Cell(15,7,$rslt1['HEURE'],1,0,'L');
		  	$pdf->Cell(38,7,$rslt1['OPERATION'].' ('.$rslt1['ID_OPERATION'].')',1,0,'L');
  ...
  $pdf->Cell(63,7,'Nombre de Mouvement : '.$nbre,1,0,'C');
  $pdf->Cell(132,7, utf8_decode('Quantité Total : ').$qtesortie,1,1,'C');
  $pdf->SetFontSize(8);
  $pdf->Output(); 
}
else
