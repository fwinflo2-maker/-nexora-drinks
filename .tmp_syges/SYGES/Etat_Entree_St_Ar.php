...
if (isset ($_SESSION['habilitation']) && ($_SESSION['habilitation']=="Administrateur" || $_SESSION['habilitation']=="Gerant"  || $_SESSION['habilitation']=="Comptable"))
{
 ...
 $pdf->AddPage();
 $pdf->SetTitle('ETAT DES ENTREES  DE STOCK ');
 $pdf->SetAuthor('SYGES');
 $pdf->SetSubject('SORTIE STOCK');
 $pdf->Ln(5);
 $pdf->SetFont('arial','B',16);
 $pdf->Write(60,'                                     ETAT DES ENTREES EN STOCK');
 $pdf->Ln(1);
 ...
 $pdf->Cell(55,7,'OPERATION',1,0,'C',1);
   $pdf->Cell(23,7,'ARTICLE',1,0,'L',1);
...
$reponse1= $DataBase->query($sql1);
		while($rslt1= $reponse1->fetch())
		{
		  	...
		  	$pdf->Cell(55,7,$rslt1['OPERATION'].' ('.$rslt1['ID_OPERATION'].')',1,0,'L');
			$pdf->Cell(23,7,$rslt1['LIBELLE'],1,0,'L');
  ...
  $pdf->Cell(58,7,'Nombre de Mouvement : '.$nbre,1,0,'C');
