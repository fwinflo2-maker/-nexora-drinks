...
if (isset ($_SESSION['habilitation']) && ($_SESSION['habilitation']=="Administrateur" || $_SESSION['habilitation']=="Gerant"  || $_SESSION['habilitation']=="Comptable"))
{
 ...
 $pdf->AddPage();
 $pdf->SetTitle('ETAT DES ENTREES EN STOCKS FRIGO');
 $pdf->SetAuthor('SYGES');
 $pdf->SetSubject('ENTREE STOCK');
 $pdf->Ln(3);
 $pdf->SetFont('arial','B',16);
 $pdf->Write(80,'                     ETAT DES ENTREES EN STOCKS FRIGO');
 $pdf->Ln(3);
 $pdf->Write(80,'                     ___________________________________');
 $pdf->Ln(45);                                
 ...
 $pdf->Cell(25,7,'QTE(BTLLE)',1,1,'C',1);
 $pdf->SetFont('Arial','',8);
 $pdf->SetTextColor(0,0,0);
  
$sql = 'SELECT DISTINCT AR.ID_ARTICLE FROM ARTICLE_RECU_FRIGO AR, APPROFRIGO A  WHERE AR.ID_APPRO=A.ID_APPRO AND A.DATE_APPRO BETWEEN "'.$Debut.'" AND "'.$Fin.'" AND A.STATUT="V"';
...
$qterecu=0;
$BTE=0;
$sql1 = 'SELECT  AR.ID_ARTICLE, AR.QTERECU, AR.NBREBTE, AR.ID_APPRO, A.ID_APPRO, A.DATE_APPRO FROM ARTICLE_RECU_FRIGO AR, APPROFRIGO A WHERE AR.ID_APPRO=A.ID_APPRO AND A.DATE_APPRO BETWEEN "'.$Debut.'" AND "'.$Fin.'" AND AR.ID_ARTICLE="'.$rslt['ID_ARTICLE'].'" AND A.STATUT="V"';
$reponse1= $DataBase->query($sql1);
		while($rslt1= $reponse1->fetch())
		{
			$qterecu=$qterecu+$rslt1['QTERECU'];
			$BTE=$BTE+$rslt1['NBREBTE'];
		}
...
$reponse2= $DataBase->query($sql2);
  ...
  $pdf->Cell(40,7,$marque.' '.$nbrebte,1,0,'L');
  $pdf->Cell(60,7,$libelle,1);
  $pdf->Cell(15,7,$qterecu,1,0,'C');
  $pdf->Cell(25,7,$BTE,1,1,'C');
