...
<title>Formulaire  d'ajout des articles à un appro cession.</title>
<style type="text/css">
label
{
	display:block;
...
<form action="CTRL/Controle_Ajout_Article_ApproCession.php" method="post" onsubmit="return verif_form()" >
<fieldset style=" width:1050px;"><legend>Ajouter des articles à un Appro Cession</legend>
<table>
<tr>
	<td><label for="codeappro">Approvisionnement *</label> </td>
    <td><input type="text" id="codeappro" name="codeappro" value="<?php echo $_GET['AP']?>" readonly="readonly" style="background:#ECECEC; width:150px;"/></td>
</tr>
<tr>
    ...
    <td><select name="codeart" id="codeart" style="width:250px;">
     <?php
	    $sql = " select id_article ,marque,libelle from article  where statut='Actif' and id_article not in (select id_article from article_recu_cession where id_appro ='".$_GET['AP']."') order by marque  ";
