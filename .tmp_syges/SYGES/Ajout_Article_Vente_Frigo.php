...
<title>Formulaire  d'ajout des articles à une vente frigo.</title>
<style type="text/css">
label
{
	display:block;
...
<form action="CTRL/Controle_Ajout_Article_Vente_Frigo.php" method="post" onsubmit="return verif_form()" >
<fieldset style=" width:1050px;"><legend>Ajouter des articles à une vente frigo</legend>
<table>
<tr>
	<td><label for="codevente">Vente *</label> </td>
    <td><input type="text" id="codevente" name="codevente" value="<?php echo $_GET['Vte']?>" readonly="readonly" style="background:#ECECEC; width:150px;"/></td>
</tr>
<tr>
    ...
    <td><select name="codeart" id="codeart" style="width:300px;">
     <?php
	    $sql = " select id_article ,marque,libelle,stockfrigo from article  where statut='Actif' and id_article not in (select id_article from articlevendu_frigo where id_sortiestock ='".$_GET['Vte']."') order by libelle  ";
