...
<title>Formulaire  d'ajout des articles à une sortie cession.</title>
<style type="text/css">
label
{
	display:block;
...
<form action="CTRL/Controle_Ajout_Article_SortieCession.php" method="post" onsubmit="return verif_form()" >
<fieldset style=" width:1050px;"><legend>Ajouter des articles à une sortie Cession</legend>
<table>
<tr>
	<td><label for="codecession">Cession *</label> </td>
    <td><input type="text" id="codecession" name="codecession" value="<?php echo $_GET['SC']?>" readonly="readonly" style="background:#ECECEC; width:150px;"/></td>
</tr>
<tr>
    ...
    <td><select name="codeart" id="codeart" style="width:250px;">
     <?php
	    $sql = " select id_article ,marque,libelle,qtestock from article  where statut='Actif' and id_article not in (select id_article from articlesortie_cession where id_sortiestock ='".$_GET['SC']."') order by libelle  ";
