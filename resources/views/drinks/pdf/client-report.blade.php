...
<body>
@include('drinks.pdf._header', ['team' => $team, 'docType' => "Chiffre d'affaires par client"])
<div class="doc-title">CA Clients</div>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-right">{{ number_format($row['total_ttc'], 0, ',', ' ') }} FCFA</td>
        </tr>
        @endforeach
    </tbody>
    <tfoot>
        <tr>
            <td>TOTAL</td>
            <td class="text-right">{{ array_sum(array_column($rows, 'sale_count')) }}</td>
            <td class="text-right">{{ number_format(array_sum(array_column($rows, 'total_ttc')), 0, ',', ' ') }} FCFA</td>
        </tr>
    </tfoot>
</table>
