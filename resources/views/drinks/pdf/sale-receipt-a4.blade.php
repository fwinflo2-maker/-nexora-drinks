...
<body>
@include('drinks.pdf._header', ['team' => $team, 'docType' => 'Facture de vente'])
<div class="doc-title">{{ $sale->code }}</div>
<div class="meta-grid">
    <div class="meta-col">
    ...
    <div class="meta-col" style="text-align:right">
        ...
        <div class="meta-value">{{ $sale->document_date?->format('d/m/Y') }}</div>
        <div class="meta-label" style="margin-top:6px">Statut</div>
        <div class="meta-value">{{ $sale->status->value === 'validated' ? 'Validé' : 'Brouillon' }}</div>
    </div>
</div>
<table>
    <thead>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="text-right">{{ number_format($line->amount_ht, 0, ',', ' ') }} FCFA</td>
            <td class="text-right">{{ number_format($line->amount_ttc, 0, ',', ' ') }} FCFA</td>
        </tr>
        @endforeach
    </tbody>
</table>
