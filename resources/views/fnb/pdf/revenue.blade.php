...
<body>
@include('fnb.pdf._header', ['team' => $team, 'docType' => 'Rapport des revenus F&B'])
<div class="doc-title">Chiffre d'affaires restauration</div>
...
<table>
    ...
    <tfoot>
        <tr>
            <td>TOTAL</td>
            <td class="text-right">{{ number_format(array_sum(array_column($rows, 'count')), 0, ',', ' ') }}</td>
            <td class="text-right">{{ number_format(array_sum(array_column($rows, 'revenue')), 0, ',', ' ') }}</td>
        </tr>
    </tfoot>
</table>
