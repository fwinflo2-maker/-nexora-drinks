...
interface FoodProfile {
    ...
    name: string;
    email: string;
    role: 'admin' | 'magasinier' | 'commercial' | 'comptable';
    status: 'active' | 'inactive';
    zones?: string[];
    joinDate: string;
}

export default function AdminFoodProfiles({ teamKpis }: { teamKpis?: any }) {
    const [searchTerm, setSearchTerm] = useState('');
    const [selectedRole, setSelectedRole] = useState<string | null>(null);

    const profiles: FoodProfile[] = [
        { id: 1, name: 'Jean Ekobe', email: 'j.ekobe@nexora.cm', role: 'admin', status: 'active', zones: ['Littoral', 'Ouest'], joinDate: '2024-01-15' },
        { id: 2, name: 'Marie Fotso', email: 'm.fotso@nexora.cm', role: 'commercial', status: 'active', zones: ['Littoral', 'Centre'], joinDate: '2024-02-10' },
        { id: 3, name: 'Alain Djoufack', email: 'a.djoufack@nexora.cm', role: 'magasinier', status: 'active', zones: ['Littoral'], joinDate: '2024-01-20' },
        { id: 4, name: 'Sophie Etaé', email: 's.etae@nexora.cm', role: 'commercial', status: 'active', zones: ['Centre', 'Ouest'], joinDate: '2024-03-01' },
        { id: 5, name: 'Pierre Ngouem', email: 'p.ngouem@nexora.cm', role: 'magasinier', status: 'active', zones: ['Littoral'], joinDate: '2024-02-15' },
        { id: 6, name: 'Francine Belinga', email: 'f.belinga@nexora.cm', role: 'comptable', status: 'inactive', zones: [], joinDate: '2024-01-10' },
    ];

    const roleConfig: Record<FoodProfile['role'], { label: string; color: string }> = {
        admin:      { label: 'Admin',              color: 'bg-purple-500/10 text-purple-400' },
        magasinier: { label: 'Gestionnaire Stock', color: 'bg-teal-500/10 text-teal-400' },
        commercial: { label: 'Vendeur',            color: 'bg-blue-500/10 text-blue-400' },
                    ...
                    <motion.div key={profile.id} variants={item} className="rounded-lg border border-border bg-card p-4 hover:shadow-lg hover:border-primary/20 transition-all duration-300 group">
                        ...
                        {profile.zones && profile.zones.length > 0 && (
                            <div className="mb-3 text-xs">
                                <div className="text-muted-foreground font-medium mb-1">Zones:</div>
                                <div className="flex flex-wrap gap-1">
                                    {profile.zones.map(z => (
                                        <span key={z} className="px-2 py-1 bg-secondary rounded text-foreground text-xs">{z}</span>
                                    ))}
                                </div>
                            </div>
                        )}
