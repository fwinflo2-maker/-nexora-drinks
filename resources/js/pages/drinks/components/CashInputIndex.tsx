...
export default function CashInputIndex({ cashInputs }: { cashInputs: any }) {
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="relative w-full sm:max-w-xs">
                    ...
                    <input
                        ...
                        placeholder="Rechercher (réf, source)..."
                        className="w-full pl-9 pr-3 py-2 bg-card border border-border rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500/50 transition-shadow"
                    />
                </div>
                <Link
                    href={route('drinks.cash-inputs.create', { current_team: team.slug })}
                    className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-medium rounded-lg transition-colors shadow-sm"
                >
                    <Plus className="h-4 w-4" /> Nouvel Apport
                </Link>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-border/50">
                            {cashInputs?.data?.length === 0 ? (
                                <tr><td colSpan={6} className="py-12 text-center text-muted-foreground">Aucun apport trouvé.</td></tr>
                            ) : (
                                cashInputs?.data?.map((input: any) => (
                                    <tr 
                                        key={input.id} 
                                        onClick={(e) => {
...
return;
                                            ...
                                            router.visit(route('drinks.cash-inputs.show', { current_team: team.slug, cashInput: input.id }));
                                        }}
                                        className="hover:bg-muted/40 transition-colors group cursor-pointer"
                                    >
                                        <td className="px-5 py-3.5 font-medium text-foreground whitespace-nowrap">{input.code}</td>
                                        <td className="px-5 py-3.5 text-muted-foreground whitespace-nowrap">{input.document_date}</td>
                                        <td className="px-5 py-3.5 text-muted-foreground">{input.source || '—'}</td>
                                        <td className="px-5 py-3.5 text-right font-medium text-emerald-500 whitespace-nowrap">+{formatCurrency(input.amount)}</td>
                                        <td className="px-5 py-3.5 whitespace-nowrap">{getStatusBadge(input.status)}</td>
                                        <td className="px-5 py-3.5 actions-cell">
                                            <div className="flex items-center justify-end gap-1 opacity-100 sm:opacity-40 group-hover:opacity-100 transition-opacity">
                                                <Link
                                                    href={route('drinks.cash-inputs.show', { current_team: team.slug, cashInput: input.id })}
            ...
            {/* Pagination */}
            {cashInputs?.links && cashInputs.links.length > 3 && (
                <div className="flex justify-center mt-2">
                    <div className="flex gap-1 bg-card p-1 rounded-lg border border-border">
                        {cashInputs.links.map((link: any, i: number) => (
                            <Link
                                key={i}
                                href={link.url || '#'}
