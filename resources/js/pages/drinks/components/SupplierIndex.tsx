...
export default function SupplierIndex({ suppliers }: { suppliers: any[] }) {
    const { currentTeam } = usePage().props;
    const team = currentTeam as Team;
    const items = Array.isArray(suppliers) ? suppliers : (suppliers as any).data || [];
    ...
    return (
        <div className="p-6">
            <div className="flex items-center justify-between mb-6">
                ...
                <Link
                    href={route('drinks.suppliers.create', { current_team: team.slug })}
                            ...
                            {items.length === 0 ? (
                                ...
                                <tr key={item.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                                    ...
                                    <td className="p-4 align-middle text-right">
                                        <Link
                                            href={route('drinks.suppliers.edit', { current_team: team.slug, supplier: item.id })}
