...
export default function PricingTierIndex({ pricingTiers }: { pricingTiers: any[] }) {
    const { currentTeam } = usePage().props;
    const team = currentTeam as Team;
    const items = Array.isArray(pricingTiers) ? pricingTiers : (pricingTiers as any).data || [];
    ...
    return (
        <div className="p-6">
            <div className="flex items-center justify-between mb-6">
                ...
                <Link
                    href={route('drinks.pricing-tiers.create', { current_team: team.slug })}
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-muted/50 border-b border-border">
                            <tr>
                                <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground">Nom</th>
                                    <th className="h-10 px-4 text-left align-middle font-medium text-muted-foreground">Taux de remise (%)</th>
                        ...
                        <tbody>
                            {items.length === 0 ? (
                                <tr>
                                    <td colSpan={4} className="p-8 text-center text-muted-foreground">Aucune donnée trouvée.</td>
                                </tr>
                            ) : items.map(item => (
                                <tr key={item.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                                    <td className="p-4 align-middle">{item.name}</td>
                                        <td className="p-4 align-middle">{item.discount_rate}</td>
                                    ...
                                    <td className="p-4 align-middle text-right">
                                        <Link
                                            href={route('drinks.pricing-tiers.edit', { current_team: team.slug, pricingTier: item.id })}
